...
//			
//			params.clear();
//			Integer stus[] = new Integer[] {STATUS_CSBH,STATUS_FSBH,STATUS_FKBH,STATUS_FKSB,ORDER_CLOSE};
...
//			List<BorrowOrder> borrowOrders = borrowOrderService.findNeedProfitPushBorrow(params);
...
//			params.put("profitPush","0");
//			params.put("userFrom", jedisCluster.get("RD_RUNDOCKIN0_ID"));
//			params.put("loanDuration", Integer.parseInt(jedisCluster.get("RD_RUNDOCKIN0_TERM") == null ? "60" : jedisCluster.get("RD_RUNDOCKIN0_TERM")));
//			params.put("releaseTime", ConstantRundocking.RELEASE_TIME);
//			List<Repayment> repayments = repaymentService.findNeedProfitPush(params);
...
//				
//				logger.info("LOSELOAN 4 back loseMoneyActionNL start count:========="+borrowOrders.size());
//				for (BorrowOrder borrowOrder : borrowOrders) { 
//					
//					if(borrowOrder == null){
//						continue;
//					}
//					BorrowOrder bo = borrowOrderService.findOneBorrow(borrowOrder.getId());//查询借款详情
//					if(bo == null){
...
//					pars.put("businessType",  ConstantRundocking.BUSINESSTYPE_NL);//1申请未放款 2是坏账 3 盈利
...
//					pars.put("intoMoney", (double)(bo.getIntoMoney() == null ? 0 : bo.getIntoMoney())/100);//实际到账金额
//					//pars.put("turnover", "0");//交易金额
//					pars.put("replayDays", DateUtil.daysBetween(bo.getCreatedAt(), new Date()));//距借款天数
////					pars.put("amount", ConstantRundocking.profitCalculation((long)bo.getMoneyAmount(),bo.getIntoMoney(),(long)0,-1
...
//					
//					String result = RDShareUtil.loseMoney(pars,rundockingOrdersService,jedisCluster);
...
//		}
//		logger.info("融都分润数据推送同步任务结束---------->loseMoneyActionNL end 用时："+(System.currentTimeMillis() - time)+"共处理： " + count);
//	}
...
//				
//				logger.info("REPAYMENT 4 back loseMoneyActionBD start count:========="+repaymentfails.size());
...
//					}
//					
//					logger.info("REPAYMENT to 4 start==========borrowId:"+repayment.getAssetOrderId());
//					BorrowOrder bo = borrowOrderService.findOneBorrow(repayment.getAssetOrderId());//查询借款详情
//					Map<String, Object> pars = new TreeMap<String, Object>();
//					pars.put("businessType",  ConstantRundocking.BUSINESSTYPE_BD);//1申请未放款 2是坏账 3 盈利
//					pars.put("timeType", bo.getLoanMethod());//类型
//					pars.put("timeLimit", bo.getLoanTerm());//期限
//					pars.put("poundage", (double)bo.getLoanInterests()/100);//服务费
//					pars.put("intoMoney", (double)(bo.getIntoMoney() == null ? 0 : bo.getIntoMoney())/100);//实际到账金额
//					//pars.put("turnover", "0");//交易金额
//					pars.put("replayDays", DateUtil.daysBetween(repayment.getCreditRepaymentTime(), new Date()));//距借款天数
////					pars.put("amount", ConstantRundocking.profitCalculation(repayment.getRepaymentAmount(),bo.getIntoMoney(),(long)0,ConstantRundocking.TD_LOANDURATION+1
////							, repayment.getRepaymentInterest()));
//					pars.put("orderNo", "REPAYMENTFAIL_"+repayment.getId());
//					pars.put("userId", re.getUserId()+"");
//					pars.put("remark", "坏账");
//					pars.put("orderTime",  DateUtil.getDateFormat("yyyy-MM-dd HH:mm:ss"));
//					pars.put("needPayment", (double)repayment.getRepaymentAmount()/100);//应还金额
//					pars.put("apr", jedisCluster.get("RD_RUNDOCKIN0_RATE"));
//					pars.put("act", ConstantRundocking.POST_FAIL);
//					
//					String result = RDShareUtil.loseMoney(pars,rundockingOrdersService,jedisCluster);
...
//		}
//		logger.info("融都分润数据推送同步任务结束---------->loseMoneyActionBD end 用时："+(System.currentTimeMillis() - time)+"共处理： " + count);
//	}
